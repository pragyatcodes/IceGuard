...
const OFFLINE_STYLE = { version: 8, name: "iceguard-offline", sources: {},
  layers: [{ id: "bg", type: "background", paint: { "background-color": "#06101f" } }] };
const OSM_STYLE = { version: 8, name: "osm", sources: { osm: { type: "raster",
  tiles: ["https://tile.openstreetmap.org/{z}/{x}/{y}.png"], tileSize: 256, maxzoom: 19,
  attribution: "OpenStreetMap contributors" } },
  layers: [{ id: "osm", type: "raster", source: "osm" }] };
const BASES = { carto: CARTO_DARK, osm: OSM_STYLE, offline: OFFLINE_STYLE };
const STATIONS = [
...
function App() {
  ...
  const [base, setBase] = useState("carto");
  ...
  useEffect(() => {
    ...
    fetch(CARTO_DARK, { mode: "cors" }).then(
      (r) => { if (!cancelled) bootMap(r.ok ? CARTO_DARK : OFFLINE_STYLE, !r.ok); },
      () => { if (!cancelled) bootMap(OFFLINE_STYLE, true); }
    );
    const bootTimer = setTimeout(() => {
      if (!cancelled && !mapRef.current) bootMap(OFFLINE_STYLE, true);
    }, 10000);
    function bootMap(style, offline) {
      if (cancelled || mapRef.current) return;
      if (offline) setMapNote("offline base map - ops layers active");
      setBase(offline ? "offline" : "carto");
      ...
      const haveLayers = () => { try { return !!mapRef.current.getSource("ice"); } catch (e) { return false; } };
      const initLayers = () => {
        if (!mapRef.current || haveLayers()) return;
        try {
      ...
      setTimeout(() => {
        if (cancelled || haveLayers()) return;
        try { map.setStyle(OFFLINE_STYLE); } catch (e) {}
  ...
  function switchBase(name) {
    setBase(name);
    const M = mapRef.current;
    if (!M) { setMapNote(""); return; }
    setMapNote(name === "offline" ? "offline base map - ops layers active" : "");
    try { M.setStyle(BASES[name]); } catch (e) {}
    // setStyle wipes custom layers: re-add once the new style is ready
    let n = 0;
    const iv = setInterval(() => {
      n++;
      const fail = () => { clearInterval(iv); setMapNote(name + " base map unreachable - try OSM or offline"); };
      try {
        if (M.getSource("ice")) { clearInterval(iv); return; }
        if (M.isStyleLoaded()) {
          try { addOpsLayers(M); setMapReady(true); } catch (e) {}
          clearInterval(iv);
        } else if (n > 30) fail();
      } catch (e) { if (n > 30) fail(); }
    }, 350);
  }
      ...
      <div className="main">
        ...
        <div className="center">
          ...
          <div className="map-legend">
            ...
            <div style={{ marginTop: 6 }}>Base map: <select value={base} onChange={(e) => switchBase(e.target.value)} style={{ background: "#081426", color: "#dbe7f5", border: "1px solid #1c2f4d", borderRadius: 4, fontSize: 11, marginLeft: 4 }}>
